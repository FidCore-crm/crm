 ...
 * "Te invitaron a usar el CRM". GoTrue inserta access_token y refresh_token
 * en el URL fragment.
 *
 * Esta página:
 *   1. Parsea el fragment
 *   2. Muestra form con dos campos: contraseña + repetir contraseña
 *   3. Al enviar, llama a POST /api/auth/aceptar-invitacion
 *   4. Si OK, redirige al dashboard (las cookies quedaron seteadas)
 */
function AceptarInvitacionContent() {
  ...
  useEffect(() => {
    ...
    if (!hash) {
      setErrorInicial('Link inválido o expirado. Pedile al administrador que te invite de nuevo.')
      return
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('')
    if (!tokens) {
      setError('Faltan datos. Pedí una invitación nueva.')
      return
    }
    if (pass1.length < 6) {
    ...
    try {
      ...
      if (json.ok) {
        ...
        setError(json.error || 'No se pudo activar la cuenta')
      }
    } catch {
          ...
          <div className="mb-8 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Definí tu contraseña para empezar
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-md flex items-start gap-2.5">
                ...
                <p className="text-xs text-emerald-800">
                  Validamos tu email. Ahora definí una contraseña para empezar a usar el CRM.
                </p>
              </div>
              <div>
