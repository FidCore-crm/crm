...
export const POST = manejarErrores(async (
  ...
  // Optimistic concurrency (#81). Si el cliente mandó if_match_updated_at,
  // exigimos coincidencia con el updated_at actual — si otro usuario cambió
  // la póliza entre medio, devolvemos 409 con registro_actual.
  if (
  ...
  if (!ESTADOS_BAJA_PERMITIDA.includes((poliza as any).estado)) {
    return respuestaError(ERRORES.NEG_OPERACION_INVALIDA, {
      detalle: `No se puede cancelar una póliza en estado ${(poliza as any).estado}. Solo se puede cancelar si está VIGENTE, PROGRAMADA o RENOVADA.`,
    })
  }
  ...
  if (siniestrosAbiertos && siniestrosAbiertos > 0) {
    return respuestaError(ERRORES.NEG_OPERACION_INVALIDA, {
      detalle: `No se puede cancelar: hay ${siniestrosAbiertos} siniestro(s) activo(s) en esta póliza. Cerralos o rechazalos primero.`,
    })
  }
  ...
  if (!motivo_baja) {
    return respuestaError(ERRORES.VALID_CAMPO_REQUERIDO, {
      campos: { motivo_baja: 'El motivo de cancelación es obligatorio' },
    })
  }
  ...
  const { error: errUpdate } = await supabase
    .from('polizas')
    .update({
      estado: ESTADO_POLIZA.CANCELADA,
      motivo_baja,
  ...
  await registrarEventoBitacora(supabase, {
    ...
    estado_nuevo: ESTADO_POLIZA.CANCELADA,
    motivo: motivo_baja,
  ...
  // Las COMPLETADAS/CANCELADAS quedan como histórico (poliza_id queda apuntando
  // a la póliza CANCELADA, que sigue existiendo).
  try {
    ...
    if (tareasElim && tareasElim.length > 0) {
      logger.info({
        modulo: 'polizas',
        mensaje: `Eliminadas ${tareasElim.length} tarea(s) activa(s) al cancelar póliza`,
        contexto: { poliza_id: id },
      })
    }
  } catch (err) {
    // No bloqueante — la cancelación ya se aplicó.
    logger.warn({
      modulo: 'polizas',
      mensaje: 'Falló eliminación de tareas al cancelar póliza',
